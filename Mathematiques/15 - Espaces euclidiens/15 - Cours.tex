...
\begin{prop}{}{11}
    ...
    \begin{equation*}
        A=\begin{pmatrix}a&c\\b&d\end{pmatrix}; \quad \begin{cases}a^2+c^2 = 1\\b^2+d^2=1\\ab+cd=0\end{cases}
    \end{equation*}
...
\begin{prop}{Réciproque du théorème spectral.}{}
    ...
    Soient $x,y \in E$. $\Lg u(x), y \Rg = \sum_{i=1}^n \Lg y_i, u(x_i) \Rg =\sum_{i=1}^n \Lg y_i, \l_i x_i \Rg$ et $\Lg x,u(y)\Rg = \sum_{i=1}^n \Lg x_i, \l_i y_i \Rg$.
\end{prop}
...
\begin{exercice}{Rayon spectral.}{}
    ...
    \boxed{2.} Soit $u\in S(E)$, d'après le théorème spectral, il existe $\B=(e_1,...,e_n)$ b.o.n. de $E$ de $\vp$ de $u$.\\
    Alors $\forall i \in \lb1,n\rb, ~ \exists \l_i \in \R, ~ u(e_i)=\l_i e_i$. Soit $x\in E$ : $u(x)=\sum_{i=1}^n\Lg \l_i,  x_i\Rg e_i$.\\
...
\begin{exercice}{Matrice de Hilbert.}{}
    Montrer que les valeurs propres de $H=\left( \frac{1}{i+j-1} \right)_{i,j}\in\M_n(\R)$ sont toutes strictement positives.
    \tcblower
...
\begin{exercice}{}{}
    ...
    Soit $\Delta=\Diag(\sqrt{\l_i})$, alors $A=P\Delta^2P^\top = P\Delta(P\Delta)^\top$ et pour $M=(P\Delta)^\top, ~ A=M^\top M$.\\
...
\begin{exercice}{}{}
    On pose $A_M=\{X^\top M X, ~ x \in \M_{n,1}(\R), ~ \|X\|=1\}$ avec $M\in\M_n(\R)$. Montrer que $A_M$ est un segment.
    \tcblower
    Si $M\in\S_n(\R)$, d'après le théorème spectral, $\exists P\in O_n(\R), ~ \exists D \in \D_n(\R), ~ A_M = PDP^\top$. Avec $D=\Diag(\l_i)$.\\
    Soit $X\in\M_{n,1}(\R)$ de norme 1. On a $X^\top MX = X^\top PDP^\top X = YDY^\top = \sum_{i=1}^n y_i^2\l_i$.\\
    On pose $\b = \max(\l_1,...,\l_n)$ et $\a=\min(\l_1,...,\l_n)$ puis :
    \begin{equation*}
        \a\sum_{i=1}^ny_i^2 \leq X^\top MX \leq \b\sum_{i=1}^n y_i^2 ~:~ \a\|Y\|^2 \leq X^\top MX \leq \b\|Y\|^2
    \end{equation*}
    Donc $\a \leq X^\top M X \leq \b$ car $\|Y\|^2=\|X\|^2=1$ donc $A_M \subset [\a, \b]$.\\
    Posons $S=\{X \in \M_{n,1}(\R), ~ \|X\|=1\}$, c'est un connexe par arcs, on pose $\phi:X\mapsto X^\top MX$ continue.\\
    Donc $\phi(S)=A_M$ est connexe par arcs, c'est un segment de $\R$ par caractérisation.\\
    Puis si $M\in\M_n(\R)$, on a $M=\frac{M^\top + M}{2}+ \frac{M-M^\top}{2}$ donc $X^\top MX = X^\top SX + X^\top AX$.\\
    Et $\X^\top AX = \Lg X,AX\Rg = \Lg AX,X\Rg = (AX^\top)X = X^\top A^\top X = -X^\top AX$ donc $X^\top AX=0$.\\
    Donc $X^\top MX = X^\top SX$ donc $A_M = A_S$.
\end{exercice}

\vspace*{-0.8cm}

\begin{exercice}{}{}
    Soit $K$ un sous-groupe compact de $\GL_n(\R)$ tel que $O_n(\R)\subset K$. Montrer que $K=O_n(\R)$.
    \tcblower
    Soit $M\in K$. Elle est inversible donc par décomposition polaire : $\exists!(\O,S)\in O_n(\R)\times\S_n^+(\R)$ telles que $M=\O S$.\\
    Alors $S=\O^\top M$, et $M\in K$, $\O^\top \in K$ donc $S\in K$. On note $\Sp(S)=\{\l_1,...,\l_n\}$ tous positifs car $S\in\S_n^+(\R)$.\\
    On pose $i_\a$ l'indice de $\a$ et $i_\b$ celui de $\b$, alors $\a = E_{i_\a} D E_{i_\a}^\top$ et $\b = E_{i_\b} D E_{i_\b}^\top$.\\
    On a $\forall k \geq 0, ~ S^k=PD^kP^\top$, avec $D^k=\Diag(\l_1^k,...,\l_n^k)$. $(S^k)_k$ bornée car suite de $K$, borné car compact.\\
    On peut en extraire $(S^{\phi(k)})_k$ convergente vers $N\in K$. On a $\forall i \in \lb1,n\rb, ~ \l_i \geq 1$ car sinon $N\notin \GL_n(\R)$ :
    \begin{equation*}
        N=P\Delta P^\top \quad \nt{avec} \quad \Delta = \Diag(\lim \l_i^{\phi(k)})_i
    \end{equation*}
    De plus, $\forall i \in \lb1,n\rb, ~ \l_i \leq 1$ car sinon, $(S^k)$ diverge et n'est donc pas bornée.\\
    Donc $\forall i \in \lb1,n\rb, ~ \l_i = 1$, donc $S=I_n$ donc $M=\O \in O_n(\R)$.
\end{exercice}

\vspace*{-0.8cm}

\begin{exercice}{}{}
    Soit $E$ euclidien, $u\in\S^+(E)$.
    \begin{enumerate}
        \item Montrer que $\exists v \in \S^+(E), ~ v^2=u$.
        \item Montrer que pour $u,v\in\S^{++}(E)$, $\det(u+v)\geq \det(u)+\det(v)$.
    \end{enumerate}
    \tcblower
    \boxed{1.} On note $(\l_1,...,\l_n)$ les valeurs propres de de $u$, toutes positives car $u\in\S^+(E)$.\\
    Par théorème spectral, il existe $(e_1,...,e_n)$ une b.o.n. de vecteurs propres de $u$ : $\forall i \in \lb1,n\rb, ~ u(e_i)=\l_i e_i$.\\
    On pose $v\in\L(E)$ tel que $v(e_i)=\sqrt{\l_i}e_i$, on a alors $v^2(e_i)=\l_i e_i = v'e_i$ donc $u=v^2$ par égalité sur une base.\\
    De plus, $v\in\S^+(E)$ car toutes ses vp sont positives.\\
    \boxed{2.} On pose $w\in\S^{++}(E), ~ w^2 = v$. On a $\det(u+v)=\det(w^2)\det(\id + w^{-1}uw^{-1})$.\\
    Soit $x\in E, ~ \Lg w^{-1} u w^{-1}(x), x \Rg = \Lg uw^{-1}(x), w^{-1}(x)\Rg \geq 0$ car $w\in\S^{++}(E)$.\\
    Montrons que pour $s\in\S^+(E)$, $\det(\id + s) \geq 1+\det(s)$. Soit $S=\Mat_\B(s)$.\\
    Par théorème spectral, $S=PDP^{\top}$ avec $P\in O_n(\R)$ et $D\in\D_n(\R)$.\\
    $\det(cd+s)=\det(I_n+S)=\det(I_n+PDP^\top)=\et(P(I_n+D)P^\top)=\det(I_n+D)=\prod_{i=1}^n(1+\l_i)\geq 1 + \prod_{i=1}^n\l_i = 1 + \det(s)$.\\
    On revient en (*) : $\det(u+v)=\det(v)\det(\id + w^{-1}uw^{-1})\geq \det(v)(1+\det(w^{-1}uw^{-1}))=\det(v)+\det(u)$
\end{exercice}

\section{Khôlles.}

\begin{exercice}{}{}
    Soit $E$ euclidien, $a\in E$ unitaire et $\forall \a \in \R, ~ f_\a : x \mapsto x + \a \Lg x, a \Rg a$ de $E$ dans $E$.
    \begin{enumerate}
        \item Montrer que $\{f_\a, \a\in\R\}$ est stable par composition.
        \item Calculer $f_\a^p$ pour tout $p\in\N$.
        \item Montrer que $f_\a\in\GL(E) \iff \a \neq - 1$.
        \item Montrer que $f_\a \in O(E) \iff \a \in \{0,-2\}$.
    \end{enumerate}
    \tcblower
    \boxed{1.} Soient $\a,\b\in\R,x\in E$. On a $f_\a\circ f_\b(x)=f_\a(x+\b\Lg x,a\Rg a) = x + \a \Lg x, a \Rg a + \b\Lg x,a\Rg a + \a\b\Lg x, a \Rg a=f_{\a+\b+\a\b}(x)$.\\
    \boxed{2.} Montrons par récurrence $\P_p$ : << $f_\a^p = f_{(\a + 1)^p-1}$ >>.\\
    \bf{Initialisation.} On a $\id = f_{\a}^0 = f_{(\a+1)^0-1}=f_0$.\\
    \bf{Hérédité.} Soit $p\in\N$ tel que $f_\a^p = f_{(\a+1)^p - 1}$. On a $f_{\a}^{p+1}=f_{(\a+1)^p - 1+\a+\a((\a+1)^p - 1)} = f_{(a+1)^{p+1}-1}$.\\
    Donc pour tout $p\in\N, ~ \P_p$.\\
    \boxed{3.} On a $\Ker(f_{-1})=\Vect(a)$ donc $f_{-1}$ n'est pas injective donc pas inversible (contraposée).\\
    Puis $\forall \a \in \R \setminus\{-1\}, ~ f_{\a}\circ f_{-\frac{\a}{1+\a}} = \id$ car $\a+\b+\a\b=0$ ssi $\b=-\frac{\a}{1+\a}$ donc $f_\a\in\GL(E)$.\\
    \boxed{4.} Soit $\a\in\R$ tel que $f_\a \in O(E)$. Soit $x\in E$. On a $\|f_\a(x)\|=\|x+\a\Lg x,a\Rg a\|$.\\
    En particulier, pour $x=a$ : $\|a\|=\|a+\a\cancel{\|a\|}a\| = |(1+\a)|\|a\|$ donc $|1+\a|=1$ donc $\a\in\{0,-2\}$.\\
    On a $f_0=\id_E$, et pour $x,y\in E, ~ \Lg f_{-2}(x), y \Rg = \Lg x, y \Rg - 2\Lg x, a\Rg \Lg y,a\Rg$ donc $f_\a^* = f_\a$.
    \begin{align*}
        f_\a \in O(E) &\iff f_\a^* = f_\a^{-1} \iff f_\a = f_\a^{-1} \iff \a = - \frac{\a}{1+\a} \\
        &\iff 1 + \a = -1 \ou \a = 0 \iff \a \in \{0,-2\}.
    \end{align*}
\end{exercice}
...
\begin{exercice}{}{}
    Soit $(E,\Lg\.,\.\Rg)$ euclidien, $f\in\L(E)$ et $(P,Q)\in\R[X]^2$ tels que $P\land Q = 1$.\\
    Montrer que $\Ker(P(f))\bot \Ker(Q(f^*))$.
    \tcblower
    Soit $(x,y)\in\Ker(P(f))\times\Ker(Q(f^*))$. Puisque $P\land Q=1$, $\exists (U,V)\in \R[X]^2, ~ PU+QV=1$.
    \begin{align*}
        \Lg x,y \Rg &= \Lg \cancel{U(f)\circ P(f)(x)} + Q(f)\circ V(f)(x), y \Rg = \Lg V(f)(x), \cancel{Q(f^*)(y)} \Rg = 0.
    \end{align*}
\end{exercice}
...
\begin{exercice}{}{}
    Soient $(A,B)\in O_n(\R)^2$. Montrer que $A \sim B \iff \X_A = \X_B$.
    \tcblower
    \boxed{\ra} Clair.\\
    \boxed{\la} Pour $\l\in\R$, $\X_A(\l)=\X_B(\l)$. Par réduction des isométries :
    \begin{equation*}
        A\sim \Diag(R(\theta_1), ..., R(\theta_r), I_p, -I_q) \et B\sim\Diag(R(\hat(\theta_1)), ... , R(\hat{\theta_s}), I_{\hat{p}}, -I_{\hat(q)}).
    \end{equation*}
    \begin{equation*}
        \X_A(\l) = \det(\l I_n - A) = (\l-1)^p(\l+1)^q\prod_{k=1}^r\X_{R(\theta_i)}(\l).
    \end{equation*}
    Pour $\theta\in~]-\pi,\pi[\setminus\{0\}$, $\X_{R(\theta)}=X^2 - 2X\cos(\theta)+1$.\\
    Par unicité de la décomposition en facteurs irréductibles, $p=\hat{p}$, $q=\hat{q}$, $r=s$, $\forall i \in \lb1,r\rb, ~ \exists j \in \lb1,r\rg, ~ \theta_i = \pm \theta_j$.\\
    Donc (?) $\Diag(R(\theta_1),...,-I_q) = \Diag(R(\hat{\theta_1}),...,-I_{\hat{q}})$ et $A\sim ... \sim B$.
\end{exercice}

\begin{exercice}{}{}
    Soit $A\in\S_n^+(\R)$. Montrer $\exists!B\in\S_n^+(\R), ~ B^2=A$.
    \tcblower
    \bf{Existence.} Théorème spectral : $\exists P \in O_n(\R), ~ A=P\Diag(\l_1,...,\l_n)P^{-1}$ où $\Sp(A)=\{\l_1,...,\l_n\}$.\\
    Posons $B:=P\Diag(\sqrt{\l_1},...,\sqrt{\l_n})P^{-1}$ (existe car $\forall i \in \lb1,n\rb, ~ \l_i \geq 0$).\\
    Alors $B^2 = A$ et $\forall i \in \lb1,n\rb, ~ \sqrt{\l_i}\geq 0$ donc $B\in\S_n^+(\R)$.\\
    \bf{Unicité.} Posons $b$ canoniquement associé à $B$. Puisque $B$ commute avec ses itérés, il commute avec $A$.\\
    Puis $A$ et $B$ sont diagonalisables donc codiagonalisables : $A=PDP^\top \et B=P\Delta P^\top$.\\
    Donc $B^2=A \iff \Delta^2 = D \iff \forall i \in \lb1,n\rb, ~ \mu_i^2 = \l_i \iff \forall i \in \ln b1,n\rb, ~ \mu_i = \sqrt{\l_i}$ car $B\in\S_n^+(\R)$.
\end{exercice}

\begin{exercice}{}{}
    Soit $E$ un espace vectoriel euclidien et $u\in\L(E)$ tel que $\Lg x,y \Rg = 0 \ra \Lg u(x), u(y)\Rg = 0$.
    \begin{enumerate}
        \item Montrer $\exists k \in \R_+, ~ \forall x \in E, ~ \|u(x)\|=k\|x\|$.
        \item En déduire que $u$ est la composée d'une homothéthie et d'un endomorphisme diagonal.
    \end{enumerate}
    \tcblower
    \boxed{1.} Déjà fait.\\
    \boxed{2.} Soit $h:x\mapsto kx$.\\
    Si $k=0$, $u=0$, n'importe quelle isométrie fonctionne.\\
    Si $k\neq0$, $v:x\mapsto \frac{1}{2}u$, alors $\forall x \in E, ~ (h\circ v)(x) = h(\frac{1}{2}u(x))=u(x)$.\\
    Alors $\|v(x)\|=\frac{1}{2}\|u(x)\|=\frac{1}{2}k\|x\|=\|x\|$.
\end{exercice}

\begin{exercice}{}{}
    Soit $M\in\S_n(\R)$ telle que $M^p = I_n$ avec $p\in\N^*$. Que dire de $M^2$ ?
    \tcblower
    D'après le théorème spectral, $M$ est orthodiagonalisable et $M\sim\Diag(\l_1,...,\l_n)$.\\
    Alors $M^p \sim \Diag(\l_1^p,...,\l_n^p)$ et $M^p = I_n$ donc $\l_i^p = 1$ puis $\l_i = \pm 1$.\\
    Puis $M^2 \sim I_n$, la seule matrice semblable à $I_n$ est $I_n$, donc $M^2=I_n$.
\end{exercice}

\pagebreak

\begin{exercice}{}{}
    Pour $P,Q\in\R[X]$, on pose
    \begin{equation*}
        \Lg P, Q \Rg = \int_0^{+\infty}P(t)Q(t)e^{-t}\dt
    \end{equation*}
    et $F=\{P\in \R_n[X], ~ P(0)\}$
    \begin{enumerate}
        \item Montrer que $\Lg\.,\.\Rg$ est bien défini et que c'est un produit scalaire.
        \item Soit $(P_1,...,P_n)$ la b.o.n. de $(1,...,X^n)$ par Gram-Schmidt. Calculer $P_k(0)^2$ pour $k\in\lb0,n\rb$.
        \item Déterminer une base de $F^\bot$, qu'on exrpimera dans $(P_0,...,P_n)$. En déduire $d(1,F^\bot)$ puis $d(1,F)$.
    \end{enumerate}
    \tcblower
    \boxed{1.} Déjà fait dans le chapitre précédent.\\
    \boxed{2.} Soit $k\in\lb0,n\rb$, alors $\deg(P_k)=k$ et en posant $u(t)=P_k(t)e^{-t}$, $v'(t)P'_k(t)$ (hyp...) :
    \begin{align*}
        \Lg P_k, P'_k \Rg &= 0 = \int_0^{+\infty}P_k(t)P'_k(t)e^{-t}\dt =  \left[ P_k^2(t)e^{-t} \right]_0^{+\infty} - \int_0^{+\infty}P_k(t)\left( P'_k(t)e^{-t} - P_k(t)e^{-t} \right)\dt\\
        &= -P_k^2(0) + \int_0^{+\infty}P_k^2(t)e^{-t}\dt = -P_k^2(0) + \|P_k\|^2 = 1 - P_k^2(0)
    \end{align*}
    Donc $P_k^2(0)=1$.
\end{exercice}
