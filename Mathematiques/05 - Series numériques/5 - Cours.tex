...
\begin{defi}{Série.}{}
    Soit $(u_n)_{n\in\N}\in E^\N$. La série de terme général $(u_n)_{n\in\N}$, notée $\sum u_n$ est la suite $(S_n)_{n\in\N}$ définie par :
    \begin{equation*}
        \forall n \in \N, \quad S_n = \sum_{k=0}^n u_k.
    \end{equation*}
    Alors $S_n$ est appelée somme partielle de la série $\sum u_n$.
\end{defi}

\begin{defi}{Convergence d'une série.}{}
    Soit $\sum u_n$ une série de terme général $(u_n)$. On note $(S_n)$ les sommes partielles.
    \begin{center}
        $\sum u_n$ converge ssi la suite des sommes partielles $(S_n)_{n\in\N}$ converge.
    \end{center}
    En cas de convergence, on notera la limite $\sum_{n=0}^{\infty}u_n$.\\
    On définit de plus le reste $R_n$ de la série : $R_n=S-S_n$ où $S=\sum_{n=0}^\infty u_n$ et $S_n=\sum_{k=0}^nu_k$.
\end{defi}
...
\begin{prop}{}{}
    ...
    \tcblower
    Soient $(u_n)\in E^\N$ et $(v_n)\in E^\N$ tels que $\exists N \in \N, ~ \forall n \geq N, ~ u_n = v_n$.\\
    On pose $S_n =\sum_{k=0}^nu_k$ et $T_n =\sum_{k=0}^n v_k$ pour tout $n\in\N$.\\
    Alors $\forall n \geq N, ~ S_n - T_n = \sum_{k=0}^{N-1}(u_k-v_k)=c$.\\
    Donc $S_n=T_n+c$, donc $(S_n)$ cv $\iff (T_n)$ cv et $\lim S_n = \lim T_n + c$.
\end{prop}
...
\begin{thm}{Comparaison des suites à termes positifs.}{}
    ...
    \boxed{3.} On a $u_n\underset{+\infty}{=}O(v_n)$ et $v_n\underset{+\infty}{=}O(u_n)$. On admet la suite du résultat.
\end{thm}
...
\begin{prop}{Séries de Riemann}{}
    ...
    Soit $\g\in\R$.
    \begin{align*}
        (n+1)^\g-n^\g &= n^\g(1+\frac{1}{n})^\g-n^\g=n^\g\left(1+\frac{\g}{n}+o\left(\frac{1}{n}\right)\right)-n^\g=\g n^{\g-1}+o(n^{\g-1})
    \end{align*}
    Donc $(n+1)^\g-n^\g\sim\g n^{\g-1}$. Or $\frac{1}{n^\a}\sim\frac{1}{1-\a}\left( (n+1)^{1-\a} - n^{1-\a} \right)$.\\
    Par théorème de comparaison (\bf{termes positifs}), leurs séries sont de mêmes natures.\\
    La deuxième est de même nature que $(n^{1-\a})_{n\in\N}$ car téléscopique, donc converge pour $\a>1$.\\
    On étudie le cas $\a=1$ (série harmonique). On a $\ln(n+1)-\ln(n)=\ln(1+\frac{1}{n})=\frac{1}{n}+o(\frac{1}{n})\sim\frac{1}{n}$.\\
    On a alors $\sum\frac{1}{n}$ et $\sum\left(\ln(n+1)-\ln(n)\right)$ de mêmes natures, puis $\sum\frac{1}{n}\sim\ln(n)$, divergentes.
\end{prop}
...
\begin{ex}{Nature de $\sum \left[ e - \left( 1 + \frac{1}{n} \right)^n \right]$.}{}
    On a déjà que $\lim \left( 1 + \frac{1}{n} \right)^n = e$ : $(1+\frac{1}{n})^n=e^{n\ln(1+\frac{1}{n})}=e^{1-\frac{1}{2n}+o(\frac{1}{n})}\to e$ par continuité de $\exp$.\\
    Le terme général de la série tend vers 0, elle n'est pas grossièrement divergente. On pose $u_n$ le t.g. de la série.\\
    On a $u_n=e-ee^{-\frac{1}{2n}+o(\frac{1}{n})}=e(1-e^{-\frac{1}{2n}+o(\frac{1}{n})})=e\left( 1-1+\frac{1}{2n}-o(\frac{1}{n}) \right)\sim \frac{e}{2n}$.\\
    Ainsi, $\sum u_n$ et $\sum \frac{e}{2n}$ sont de mêmes natures (théorème de comparaison à \bf{termes positifs}), ici divergentes...
\end{ex}
...
\begin{rappel}{Séries géométriques.}{}
    ...
    \end{equation*}
    \tcblower
    Si $|a|\geq1$, la série diverge grossièrement.\\
    Sinon, $(1-a)\sum_{k=0}^na^k=\sum_{k=0}^na^k-\sum_{k=0}^na^{k+1}=1-a^{n+1}\to1$ donc $S_n\to\frac{1}{1-a}$.\\
    Puis $\sum_{n+1}^\infty a^k = a^{n+1}\left( \sum_{k=0}^\infty a^k \right)=\frac{a^{n+1}}{1-a}$.
\end{rappel}
...
\begin{defi}{}{}
    Soit $(u_n)_{n\in\N}\in E^\N$, alors $\sum u_n$ est absolument convergente (AC) ssi $\sum|u_n|$ converge.
\end{defi}
...
\begin{ex}{}{}
    \begin{enumerate}
        ...
        \item $\sum\frac{(-1)^n}{n^2}$ converge car absolument convergente, ou CSSA.
    \end{enumerate}
\end{ex}
...
\begin{exi}{Lemme d'Abel}{}
    ...
    \bf{Remarque.} Étude des séries $\sum\frac{e^{in\theta}}{n^\a}$ avec $\a\in\R$ et $\theta\in\R$.\\
